    ...
    if(isset($_SESSION['state'])) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($_POST['daate'] != 0) {
                ...
                if($quarter == 1) {
                    $querydate = " AND daate IN ('01-$date' ,'02-$date', '03-$date') AND state = " . $_SESSION['state'];


                    $query2 = " AND daate IN ('01-$date' ,'02-$date', '03-$date') ";
                } elseif ($quarter == 2) {
                    $querydate = " AND daate IN ('04-$date' ,'05-$date' ,'06-$date') AND state = " . $_SESSION['state'];

                    $query2 = " AND daate IN ('04-$date' ,'05-$date' ,'06-$date') ";
                } elseif ($quarter == 3) {
                    $querydate = " AND daate IN ('07-$date', '08-$date', '09-$date') AND state = " . $_SESSION['state'];

                    $query2 = " AND daate IN ('07-$date', '08-$date', '09-$date') ";
                } elseif ($quarter == 4) {
                    $querydate = " AND daate IN ('10-$date', '11-$date', '12-$date') AND state = " . $_SESSION['state'];
