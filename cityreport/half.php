    ...
    if(isset($_SESSION['city'])) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($_POST['daate'] != 0) {
                ...
                if($half == 1) {
                    $querydate = " AND daate IN ('01-$date', '02-$date', '03-$date', '04-$date', '05-$date', '06-$date') AND city = " . $_SESSION['city'];


                    $query2 = " AND daate IN ('01-$date', '02-$date', '03-$date', '04-$date', '05-$date', '06-$date') ";

                } elseif ($half == 2) {
                    $querydate = " AND daate IN  ('07-$date' ,'08-$date', '09-$date', '10-$date', '11-$date' , '12-$date') AND city = " . $_SESSION['city'];
