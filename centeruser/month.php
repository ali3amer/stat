<?php
  ...
  if(isset($_SESSION['centeruser']) && isset($_SESSION['center'])) {
      ...
      $daate = (isset($_GET['daate']) && is_string($_GET['daate'])) ? $_GET['daate'] : 0;
      ...
      <div class="container">
        ...
        <?php
        $visitsid  = getAllFrom('ID, daate', 'visit', 'WHERE client = ' . $client['ID'], " AND vmonth = '$daate' ", 'ID', '');
        if($visitsid != null) { ?>
        ...
        foreach($visitsid as $visitid) { ?>
            ...
            <li><a  style="color: #333;" data-toggle="tab" href="#<?php echo $visitid['daate']  ?>"><?php echo $visitid['daate'] ?></a></li>
        <?php
        } ?>
        </ul>
      ...
      foreach($visitsid as $visitid):
          $summ = 0;
        $meets  = getAllFrom('meet.*, tables.name as name', 'meet', 'INNER JOIN tables ON tables.ID = meet.meet WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $checks = getAllFrom('checks.*, tables.name as name', 'checks', 'INNER JOIN tables ON tables.ID = checks.checks WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $ills  = getAllFrom('ills.*, tables.name as name', 'ills', 'INNER JOIN tables ON tables.ID = ills.ill WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $proccess = getAllFrom('proccess.*, tables.name as name', 'proccess', 'INNER JOIN tables ON tables.ID = proccess.procces WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $eyes  = getAllFrom('eyes.*, tables.name as name', 'eyes', 'INNER JOIN tables ON tables.ID = eyes.eye WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $defrents  = getAllFrom('defrent.*, tables.name as name', 'defrent', 'INNER JOIN tables ON tables.ID = defrent.defrent WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $profs  = getAllFrom('prof.*, tables.name as name, profsname.name as profname', 'prof', 'INNER JOIN tables ON tables.ID = prof.prof INNER JOIN profsname ON profsname.ID = prof.profname WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $sugers  = getAllFrom('suger.*, tables.name as name', 'suger', 'INNER JOIN tables ON tables.ID = suger.suger WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        $medcins  = getAllFrom('*', 'medcin', 'WHERE visit = ' . $visitid['ID'], '', 'ID', '');
        ?>

        <div id="<?php echo $visitid['daate']  ?>" class="tab-pane fade">
